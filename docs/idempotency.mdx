...
export const initiateRefund = task({
  ...
  run: async (payload: {
    ...
    // refundId must identify one unique business operation, not a random value.
    const operationId = `refund:${payload.refundId}`;
    const idempotencyKey = await idempotencyKeys.create(operationId, { scope: "global" });
    ...
    await refundPayment.trigger(
      ...
      {
        // Trigger.dev deduplicates this child task trigger across parent runs.
        idempotencyKey,
      }
    );
  },
...
<Warning>
Stripe can prune idempotency keys after 24 hours. If a refund workflow can retry later, persist the business operation and Stripe refund ID, then reconcile with Stripe before issuing another refund.
</Warning>
